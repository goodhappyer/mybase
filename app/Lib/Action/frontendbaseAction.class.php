...
class frontendbaseAction extends basecommonAction
{
	...
	public function _initialize ()
	{
		parent::_initialize();
		$access = C("ACCESS");
		if ($this->token || ! ($access[GROUP_NAME . '/' . MODULE_NAME . '/' . ACTION_NAME] === true || $access[GROUP_NAME . '/' . MODULE_NAME . '/*'] === true || $access[GROUP_NAME . '/*/*'] === true))
		{
			$this->_init_visitor();
		}
	}
	...
	private function _init_visitor ()
	{
		...
		{
			/* 如果换成数字，就一定是正常用户 */
			$this->visitor = D("user")->where(array ("id" => $id ,'last_time' => $id_last_time[1]))->find();
			if (! $this->visitor)
			{
